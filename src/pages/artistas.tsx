...
function verificarLink() {
    var redeSocialElement = document.getElementById('rede-social') as HTMLInputElement;
    var linkElement = document.getElementById('link-rede-social') as HTMLInputElement;
    ...
    if (!link.toLowerCase().startsWith(redeSocial.toLowerCase())) {
        errorMessage.textContent = "O link fornecido não corresponde à rede social selecionada.";
    } else {
...
const handleSenhaInput = () => {
    ...
    if (!senhaInput || !errorMessage || !uppercaseReq || !lowercaseReq || !numberReq || !specialReq || !lengthReq || !successGif || !requirementsLabel) {
        return;
    }
    ...
    if (senha.length < 8 || !regexUpperCase.test(senha) || !regexLowerCase.test(senha) || !regexNumbers.test(senha) || !regexSpecialChars.test(senha)) {
...
const Artista: React.FC = () => {
    ...
    useEffect(() => {
        ...
        document.getElementById('link-rede-social')?.addEventListener('input', verificarLink);
        document.getElementById('senha')?.addEventListener('input', handleSenhaInput);

        return () => {
            document.getElementById('file-upload')?.removeEventListener('change', handleFileUploadChange);
            document.getElementById('link-rede-social')?.removeEventListener('input', verificarLink);
            document.getElementById('senha')?.removeEventListener('input', handleSenhaInput);
        };
    }, []);
        ...
        <>
            ...
            <div className="card">
                ...
                <div className="actions">
                    <button onClick={handleArtista} className="form-btn">criar conta</button>
